...
static void Realize(Widget w,XtValueMask *valueMask,XSetWindowAttributes *attrs){
  ...
  if(msw->msgwin.is_shaped == FALSE) ShapeWindow(msw);
  DrawFrame(msw);
...
static Boolean SetValues(Widget current,Widget request,Widget new,ArgList args,Cardinal*  num_args){
  ...
  if(XtIsRealized((Widget)mswnew)){
      if(mswnew->msgwin.is_shaped == FALSE) ShapeWindow(mswnew);
      DrawFrame(mswnew);
  }
  return ret;
...
static void SetWindowLocate(MsgwinWidget msw){
  ...
  while(XtParent(top)) top = XtParent(top);
  ...
  if(main_x > DisplayWidth(XtDisplay(top),0)) main_x -= (Dimension)(-1);
  if(main_y > DisplayHeight(XtDisplay(top),0)) main_y -= (Dimension)(-1);
  ...
  if(Old != WindowMode) msw->msgwin.is_shaped = FALSE;
  msw->msgwin.WindowMode = WindowMode;
}

static void ManageChild(Widget parent){
  int Longest_width,Form_height,i,ypos,b_ypos,tmp_width,b_height,b_width,tmp_height;
  int same_line;
  ...
  for(i=0;i<msw->composite.num_children;i++){
	...
	tmp_width = child->core.width + child->core.x; /* mct->form.dx; */
