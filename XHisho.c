...
#define PIXATT  (xhw->xhisho.xpmatt)
...
extern int LoadBmp(Widget,GC*,Pixmap*,char*,int*,int*,int,Boolean);
...
static void Realize(Widget w,XtValueMask *valueMask,XSetWindowAttributes *attrs){
  ...
  XtResizeWidget(XtParent(xhw),PIXATT.width
		 ,PIXATT.height + clock_height,FRAME_WIDTH);
  XtResizeWidget((Widget)xhw,PIXATT.width,PIXATT.height + clock_height
		 ,FRAME_WIDTH);
}
...
static void Redraw(Widget w,XEvent* event,Region region){
  ...
  XCopyArea(DISPLAY,PIXMAP,WINDOW,XH_GC,0,0,PIXATT.width,PIXATT.height,0,0);
...
static void ClockDraw(XHishoWidget xhw){
  ...
  if(xhw->xhisho.c_draw){
    ...
    XFillRectangle(DISPLAY,WINDOW,XH_GC,0,PIXATT.height,PIXATT.width
		   ,xhw->core.height - PIXATT.height);
