...
static void PrevMonth(Widget w,caddr_t client_data,caddr_t call_data){
  ...
  if(t_month < 0) t_month = 11;
  if(t_day > wdays[t_month]) t_day = wdays[t_month];
...
Widget CreateCalendarWindow(Widget w,int Month,struct tm tm_now){
  ...
  time_t now;
  struct tm *tm_tmp;
  int i,j,k,l,m,tmp_width,Longest_label,calendar_offset,NowYear,NowMonth,NowDay;
  ...
  */
  ...
  for(i = 0;i < 6;i++){
	...
	l++;
	if(wdays[Month] + uru_adjust < l - 1) break;
	labelargs[0].value = (XtArgVal)tmpstring;
