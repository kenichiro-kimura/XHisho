...
static int WriteSchedule(struct tm tm_now){
  ...
  for(i = 0;i < MAX_SCHED_NUM;i++){
    ...
    if(strlen(dbuf) > BUFSIZ) dtmp = realloc(dtmp,strlen(dbuf) +1);
    if(strlen(ebuf) > BUFSIZ) etmp = realloc(etmp,strlen(dbuf) +1);
...
static int CheckIsSchedPast(int i,Schedule* sc){
  ...
  */
  ...
  if(i >= MAX_SCHED_NUM || i < 0) return(0);
  time(&sched);
  tmp = localtime(&sched);
...
static int ScheduleComp(Schedule* a,Schedule* b){
  if(atoi(a->hour) > atoi(b->hour)) return 1;
  if(atoi(a->hour) < atoi(b->hour)) return -1;
  if(atoi(a->min) > atoi(b->min)) return 1;
  return -1;
}
