...
static unsigned char* EUC2SJIS(unsigned char*);
...
void ORParser(char* in)
{
  ...
  while(chr_ptr != NULL && (next_ptr = strchr(in_buffer,')'))!= NULL){
    ...
    tmp_buffer[chr_ptr - in_buffer] = '\0';
    strncpy(tmp_string,in_buffer,next_ptr - in_buffer);
    tmp_string[next_ptr - in_buffer] = '\0';
...
static unsigned char* EUC2SJIS(unsigned char* in) {
  ...
  *out_ptr = '\0';
  ...
  out_ptr = (unsigned char*)strdup(ret);
  free(ret);
...
static unsigned char* UTF82EUC(unsigned char* in)
{
  ...
  for(chr_ptr = in;*chr_ptr;chr_ptr++){
    if(*chr_ptr >0 && *chr_ptr < 0x7F){
      /* this is ASCII */
      *out_ptr++ = *chr_ptr;
    } else {
      u1 = *chr_ptr++;
...
static unsigned int UNICODE2EUC(unsigned int ucode)
{
  ...
  for(i = 0;table[i].jis;i++)
    if(table[i].ucode == ucode) return table[i].jis + 0x8080;
...
static unsigned char* SSTPParser(unsigned char* in)
{
  messageBuffer kbuf;
  unsigned char* buffer;
  unsigned char* chr_ptr;
